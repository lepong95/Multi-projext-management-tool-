...
const CommandPalette = {
    ...
    init() {
        document.addEventListener('keydown', (e) => {
            ...
            if (this.isOpen && ['ArrowDown', 'ArrowUp', 'Enter'].includes(e.key)) {
                e.preventDefault(); this.navigate(e.key);
            }
        });
        document.getElementById('commandPaletteInput').addEventListener('input', () => this.filter());
        document.getElementById('commandPaletteModal').addEventListener('click', (e) => {
            if (e.target.id === 'commandPaletteModal') this.close();
        });
    },
    ...
    generateCommands() {
        ...
        document.querySelectorAll('.sidebar-link[data-view]').forEach(link => {
            this.commands.push({
                ...
                title: `前往 ${link.textContent.trim()}`, path: '導航',
                action: () => { link.click(); this.close(); }
            });
        });
        this.commands.push({ icon: '➕', title: '新增任務', path: '操作', action: () => { addNewTask(); this.close(); } });
        const project = getCurrentProject();
        if (project && project.tasks) {
            project.tasks.forEach(task => {
                this.commands.push({
                    icon: '🗂️', title: `開啟任務: ${task.taskName}`, path: `專案: ${project.name}`,
                    action: () => { editTask(task.id); this.close(); }
                });
            });
        }
    },
    filter() {
        ...
        resultsContainer.innerHTML = filteredCommands.map((cmd, index) => `
            <div class="cp-item ${index === 0 ? 'selected' : ''}" data-index="${index}">
    ...
    navigate(key) {
        ...
        if (!items.length || items[0].textContent === '找不到結果。') return;
        items[this.selectedIndex].classList.remove('selected');
        if (key === 'ArrowDown') this.selectedIndex = (this.selectedIndex + 1) % items.length;
        else if (key === 'ArrowUp') this.selectedIndex = (this.selectedIndex - 1 + items.length) % items.length;
        else if (key === 'Enter') this.execute();
    ...
    execute() {
        const input = document.getElementById('commandPaletteInput').value.toLowerCase();
        const filteredCommands = this.commands.filter(cmd => cmd.title.toLowerCase().includes(input) || cmd.path.toLowerCase().includes(input));
        if (filteredCommands[this.selectedIndex]) filteredCommands[this.selectedIndex].action();
    }
};
...
function showPromptModal(promptText, title) {
    ...
    newCopyButton.addEventListener('click', copyButtonClickHandler);
...
function autofillTaskDetails() {
    ...
    const prompt = `
Analyze the following user input for a task and return a JSON object with suggestions for "taskName", "taskPurpose", "priority", and "taskDueDate".
- "taskName" should be a concise, formal version of the user's input.
- "taskPurpose" should be a brief, one-sentence goal.
- "priority" can be "low", "medium", "high", or "urgent".
- "taskDueDate" should be calculated if relative terms are used (e.g., "tomorrow", "next Friday"). Today's date is ${new Date().toISOString().split('T')[0]}.
- If a field cannot be determined, return an empty string for it.

User Input: "${taskName}"

Respond with ONLY a single, valid JSON object in the format:
{"taskName": "...", "taskPurpose": "...", "priority": "...", "taskDueDate": "YYYY-MM-DD"}`;
    showPromptModal(prompt, '欄位自動填寫提示');
}
...
Respond with ONLY a single, valid JSON object with a single key "steps", which is an array of strings.

Example format:
{"steps": ["First step description", "Second step description", "Third step description"]}`;
    showPromptModal(prompt, '行動計畫產生提示');
}
...
Return ONLY the estimated time as a plain string.`;
    showPromptModal(prompt, '預計耗時估算提示');
}
...
Respond with ONLY a single, valid JSON object with two keys: "riskLevel" (a single word: "Low", "Medium", or "High") and "reason" (a brief, one-sentence explanation in Traditional Chinese).
Example: {"riskLevel": "High", "reason": "此任務優先級高且有多個依賴項，延誤風險較大。"}`;
    showPromptModal(prompt, '風險評估提示');
        then(response => {
            try {
                const result = JSON.parse(response);
                document.getElementById('riskAssessmentResult').textContent = result.reason;
                document.getElementById('taskRisk').value = result.riskLevel;
                
                // Update the visual style
                const resultDiv = document.getElementById('riskAssessmentResult');
                resultDiv.className = 'risk-assessment-result';
                resultDiv.classList.add(`risk-${result.riskLevel.toLowerCase()}`);
            } catch (error) {
                console.error('Error parsing risk assessment result:', error);
                showNotification('風險評估結果格式無效', 'error');
            }
        })
        .catch(error => {
            console.error('Risk assessment failed:', error);
            showNotification('風險評估失敗', 'error');
        });
}
...
function generateWeeklyReflection() {
    ...
    let weeklyData = { completedTasks: [], overdueTasks: [], createdTasks: 0 };

    allProjects.forEach(project => {
        (project.tasks || []).forEach(task => {
            if (new Date(task.created) >= oneWeekAgo) weeklyData.createdTasks++;
            if (task.status === '已完成') {
                const completionEntry = (task.history || []).find(h => h.note.includes('已完成'));
                if (completionEntry && new Date(completionEntry.date) >= oneWeekAgo) {
...
- Tasks completed: ${weeklyData.completedTasks.length} (${weeklyData.completedTasks.join(', ') || 'None'})
- Tasks currently overdue: ${weeklyData.overdueTasks.length} (${weeklyData.overdueTasks.join(', ') || 'None'})
...
2.  **挑戰 (Challenges):** Point out potential issues, like overdue tasks, in a constructive way.
3.  **建議 (Suggestions):** Offer one or two simple, actionable suggestions for the upcoming week.`;
    showPromptModal(prompt, '每週反思產生提示');
}
    ...
    allProjects.forEach(project => {
        (project.tasks || []).forEach(task => {
            ...
            if (isDueToday || isOverdue) {
                tasksForToday.push({ name: task.taskName, priority: task.priority, project: project.name, isOverdue: isOverdue });
            }
        });
    });

    if (tasksForToday.length === 0) {
        showNotification('🎉 今天沒有到期或逾期的任務！', 'success');
        return;
    }
...
Tasks:
${JSON.stringify(tasksForToday, null, 2)}
...
3.  For each task, briefly explain *why* it's a priority (e.g., "這是緊急任務且已逾期").
4.  End with a short, motivating closing sentence.`;
    showPromptModal(prompt, '今日焦點建議提示');
}
...
function setupAllEventListeners() {
    ...
    // NEW: Add listener for the delete button
    ...
    /// Manual Action Plan Button
    document.getElementById('addManualStepButton')?.addEventListener('click', addManualActionPlanStep);
...
function renderDashboardView(viewType) {
    ...
    // Set active button and render appropriate view
    switch(viewType) {
        case 'briefing':
            document.getElementById('showMorningBriefing')?.classList.add('active');
            renderMorningBriefing();
            break;
        case 'review':
            document.getElementById('showEodReview')?.classList.add('active');
            renderEndOfDayReview();
            break;
        case 'standard':
            document.getElementById('showStandardDashboard')?.classList.add('active');
...
function renderMorningBriefing() {
    ...
    // Gather tasks for today and overdue
    allProjects.forEach(project => {
        (project.tasks || []).forEach(task => {
    ...
    let briefingHTML = `
        <div class="card morning-briefing-card">
            ...
            <div class="briefing-summary">
                <p>今天是 ${new Date().toLocaleDateString('zh-TW', { 
                    year: 'numeric', 
                    month: 'long', 
                    day: 'numeric', 
                    weekday: 'long' 
                })}。</p>
                <p>您今天有 ${criticalTasks.length} 個需要關注的任務：</p>
                ${criticalTasks.length > 0 ? `
                    <ul style="margin-top: 1rem;">
                        ${criticalTasks.map(task => `
                            <li class="briefing-task-item" data-task-id="${task.id}" data-project-id="${task.projectId}">
                                ${task.status === 'Overdue' ? '🚨' : '📌'} 
                                ${escapeHtml(task.name)} 
                                (${task.status === 'Overdue' ? '已逾期' : '今日到期'})
                                <br>
                                <small style="color: #666;">專案：${escapeHtml(task.project)}</small>
                            </li>
                        `).join('')}
                    </ul>
                ` : '<p>今天沒有緊急任務。</p>'}
    ...
    dashboardContent.innerHTML = briefingHTML;
}
...
function renderEndOfDayReview() {
    ...
    allProjects.forEach(project => {
        (project.tasks || []).forEach(task => {
            ...
            if (task.status === '已完成') {
                const completionDateEntry = (task.history || []).find(h => 
                    h.note.includes('已完成') && h.date === todayStr
                );
                if (completionDateEntry) {
                    dailyStats.completedToday.push({
                        name: task.taskName,
                        project: project.name
                    });
                }
                ...
                if (['內容準備中', '設計中', '待審批'].includes(task.status)) {
                ...
                if (task.taskDueDate === tomorrowStr) {
                    dailyStats.dueNextDay.push({
                        name: task.taskName,
                        project: project.name
                    });
                }
    ...
    let reviewHTML = `
        <div class="card eod-review-card">
            ...
            <div class="briefing-summary">
                <h3>今日完成</h3>
                ${dailyStats.completedToday.length > 0 ? `
                    <ul>
                        ${dailyStats.completedToday.map(task => `
                            <li>${escapeHtml(task.name)} (${escapeHtml(task.project)})</li>
                        `).join('')}
                    </ul>
                ` : '<p>今天尚未完成任何任務。</p>'}
                ...
                <h3>工作進度</h3>
                <ul>
                    <li>進行中的任務：${dailyStats.inProgress} 個</li>
                    <li>待處理的任務：${dailyStats.remainingTasks} 個</li>
                </ul>

                ${dailyStats.dueNextDay.length > 0 ? `
                    <h3>明天到期的任務</h3>
                    <ul>
                        ${dailyStats.dueNextDay.map(task => `
                            <li>${escapeHtml(task.name)} (${escapeHtml(task.project)})</li>
                        `).join('')}
                    </ul>
                ` : ''}
    ...
    dashboardContent.innerHTML = reviewHTML;
}
...
function deleteTask() {
    ...
    if (confirm('Are you sure you want to permanently delete this task? This action cannot be undone.')) {
        ...
        // 1. Remove the task itself from the project's task list
        project.tasks = project.tasks.filter(t => t.id != taskId);

        // 2. Clean up dependencies: Remove this task's ID from any other task that depends on it
        project.tasks.forEach(task => {
            if (task.dependencies && task.dependencies.includes(parseInt(taskId))) {
        ...
        // 4. Close the modal and refresh the user interface
        closeModal('taskModal');
        renderTaskBoard();
        renderDashboard();
        refreshCalendar();
        
        showNotification('Task was successfully deleted.', 'success');
    }
}
...
function renderActionPlan(task) {
    ...
    if (!task.actionPlan || task.actionPlan.length === 0) {
        container.innerHTML = '<p>點擊按鈕以使用 AI 生成建議的步驟清單。</p>';
        return;
    }

    container.innerHTML = task.actionPlan.map((step, index) => `
    ...
    container.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            const stepIndex = parseInt(checkbox.id.replace('step', ''));
            task.actionPlan[stepIndex].completed = checkbox.checked;
            const project = getCurrentProject();
            saveProject(project);
        });
    });
...
function addManualActionPlanStep() {
    const container = document.getElementById('actionPlanContainer');
    const input = document.createElement('input');
    input.type = 'text';
    input.placeholder = '輸入新的步驟...';
    input.className = 'action-plan-input';
    ...
    input.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            ...
            if (stepText) {
                ...
                if (!task.actionPlan) task.actionPlan = [];
                task.actionPlan.push({ text: stepText, completed: false });
...
function saveTask(e) {
    ...
    const taskData = {
        ...
        riskAssessment: { // Save from the manual dropdown
            riskLevel: document.getElementById('taskRisk').value || 'Low',
            reason: '手動設定'
        } // This was the closing brace for riskAssessment
    };
